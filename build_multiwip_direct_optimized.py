# -*- coding: utf-8 -*-
r"""
사내 DB 원천을 직접 조회하여 중간 CSV 없이 f1/f2/f3를 생성한다.
...
실행 예
    python build_multiwip_direct_optimized.py
    python build_multiwip_direct_optimized.py --export-excel
...
WORK_DB_PATH = OUTPUT_DIR / "multiwip_work.duckdb"
...
def canonicalize_dataframe_columns(df: pd.DataFrame, expected: Iterable[str], table_name: str) -> pd.DataFrame:
    """비슷한 컬럼명을 expected 이름으로 정규화한다. 다중 후보는 오류 처리한다."""
...
def get_data(query: str, label: str, stats: RunStats) -> pd.DataFrame:
    ...
    if not isinstance(result, pd.DataFrame):
        try:
            ...
            stats.warnings.append(f"{label}: getData 반환형을 pandas DataFrame으로 변환했습니다: {type(result)}")
        except Exception as exc:
            raise TypeError(f"{label}: getData 반환값을 pandas DataFrame으로 변환할 수 없습니다: {type(result)}") from exc
...
# 원천 조회 및 staging
# =============================================================================
def load_sources(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    # 작은 공통 원천부터 적재
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DB 직접 조회 기반 MultiWIP f1/f2/f3 생성")
...
def configure_duckdb(con: duckdb.DuckDBPyConnection) -> None:
    ...
    threads = os.environ.get("MULTIWIP_DUCKDB_THREADS")
    memory_limit = os.environ.get("MULTIWIP_DUCKDB_MEMORY_LIMIT")
    if threads:
...
def main(argv: list[str] | None = None) -> None:
    ...
    ensure_dirs()
    if args.rebuild and WORK_DB_PATH.exists():
        WORK_DB_PATH.unlink()
    ...
    try:
        ...
        print(f"[DONE] 작업 DB: {WORK_DB_PATH}")
