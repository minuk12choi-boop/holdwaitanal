...
def filter_step_dataframe_to_target_lots(
    df: pd.DataFrame,
    target_lot_ids: set[str],
    line: str,
    stats: RunStats,
) -> pd.DataFrame:
    """StepPath는 DB에서 넓게 조회하되, 반환 직후 현재 재공 LOT만 로컬 필터링한다.

    원천 SQL의 WHERE 조건은 건드리지 않는다. 기존 build_step에서 결국 current_lot과
    inner join으로 제거되던 행을 DuckDB 적재 전에 제거하여 작업 DB와 후속 연산량만 줄인다.
    """
    before = len(df)
    lookup = {normalize_column_token(c): c for c in df.columns}
    lot_col = lookup.get(normalize_column_token("lot_id"))
    if lot_col is None:
        raise RuntimeError(f"{line} StepPath: lot_id 컬럼을 찾을 수 없습니다")
    if not target_lot_ids:
        stats.validations[f"{line}_step_local_filter_before"] = before
        stats.validations[f"{line}_step_local_filter_after"] = before
        stats.validations[f"{line}_step_local_filter_removed"] = 0
        return df
    mask = df[lot_col].astype("string").isin(target_lot_ids)
    ...
    print(f"[FILTER] {line} StepPath current LOT only: {before:,} -> {after:,} ({before-after:,} removed)")
    del df
    gc.collect()
    return filtered
...
def load_sources(con: duckdb.DuckDBPyConnection, stats: RunStats, smoke_test: bool = False) -> None:
    # Lot은 작으므로 전체 조회한다. full에서도 라인별 대상 LOT_ID를 확보해 StepPath 로컬 선필터에 사용한다.
    df = get_data(LOT_QUERY, "lot", stats)
    lot_lookup = {normalize_column_token(c): c for c in df.columns}
    line_col = lot_lookup.get(normalize_column_token("line"))
    lot_col = lot_lookup.get(normalize_column_token("lot_id"))
    target_lot_ids: dict[str, set[str]] = {"KFR7": set(), "PFR1": set()}
    if line_col is not None and lot_col is not None:
        for line in ("KFR7", "PFR1"):
            target_lot_ids[line] = set(
                df.loc[df[line_col].astype(str).eq(line), lot_col]
                .dropna().astype(str).tolist()
            )
    smoke_lot_ids = pick_smoke_lot_ids(df) if smoke_test else {"KFR7": [], "PFR1": []}
    if smoke_test:
        # 실제 LOT 값은 로그 파일에 남기지 않는다. 개수만 기록한다.
        stats.validations["smoke_lot_count_by_line"] = {k: len(v) for k, v in smoke_lot_ids.items()}
    stage_dataframe(
    ...
    # 공통 dimension은 작고 downstream 매칭률에 중요하므로 전체 조회한다.
    ...
    for line, query, table in (
        ...
        # 중요: 원천 SQL은 그대로 두고, 반환된 DataFrame에서 현재 재공 LOT만 선필터한다.
        df = filter_step_dataframe_to_target_lots(df, target_lot_ids.get(line, set()), line, stats)
        stage_dataframe(
            con, df, table, stats,
            expected_columns=[
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        with timer(stats, "Tip 전처리"):
            build_tip(con, stats)
            # 검증에 필요한 tip_cham과 최종 t만 남긴다.
            drop_tables(con, ["tip_kfr7_raw", "tip_pfr1_raw", "tip_raw_union", "tip_t", "tip_ttt", "tip_te", "tip_main", "tip_tee", "equipment_status"])
