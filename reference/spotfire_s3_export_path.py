"""
spotfire_s3_export_path.py — S3 적재 (분할 path)

  STEP_PATH (700만행). 혼자 두어야 메모리가 버틴다.
...
[등록]
  Tools > Register Data Functions
    ...
    Input Parameters  : 아래 1개만 Table 로 등록
                        PFR1_KFR7_STEP_PATH
    Output Parameters : upload_log_path (Table)
...
# boto3 설치 전 점검용. False 로 두면 업로드하지 않고 환경 정보만 upload_log_path 에
# 남긴다(파이썬 경로 / 설치 명령).
USE_BOTO3 = True
# ────────────────────────────────────────────────────────────────────────────
