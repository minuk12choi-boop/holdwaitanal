...
[조각 수는 여기 적지 않는다]
  ...
  찾아 번호 순으로 돌린다. 3등분을 6등분으로 바꿔도 이 스크립트는
  고칠 필요가 없다.
  ...
  읽는 쪽이 중간 상태를 최신으로 오인하지 않는다.
>>>>>>> 1888235 (Support any number of split parts and add six-way export scripts)
...
<<<<<<< HEAD
        #   여기서 '완료' 는 **함수가 예외 없이 끝났다** 는 뜻일 뿐이다.
        #   업로드 성공 여부는 upload_log_* 표의 status 로 확인한다.
        #   (스크립트가 오류를 잡아 FAIL 행으로 적고 정상 종료하기 때문)
        log("완료 (테이블 적재결과는 upload_log 확인 · 함수 %d개)" % n)
=======
