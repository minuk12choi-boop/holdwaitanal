"""
spotfire_s3_export_path_10.py — S3 적재 (s3drive_path_10)

  STEP_PATH 11/11 조각

[등록]
  Tools > Register Data Functions
    Name              : s3drive_path_10
    Type              : Python script
    Input Parameters  : 아래 1개만 Table 로 등록
                        PFR1_KFR7_STEP_PATH_10
    Output Parameters : upload_log_path_10 (Table)
...
# boto3 설치 전 점검용. False 로 두면 업로드하지 않고 환경 정보만 upload_log_path_10 에
# 남긴다(파이썬 경로 / 설치 명령).
USE_BOTO3 = True
# ────────────────────────────────────────────────────────────────────────────
