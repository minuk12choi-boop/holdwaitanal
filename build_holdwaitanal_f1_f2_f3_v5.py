...
실행 예
    ...
    python build_holdwaitanal_f1_f2_f3_v5.py --rebuild --export-excel
...
def filter_step_dataframe_to_needed_rows(
    df: pd.DataFrame,
    current_order_by_lot: dict[str, int],
    line: str,
    stats: RunStats,
) -> pd.DataFrame:
    """StepPath 원천 SQL은 그대로 유지하되 DuckDB 적재 전 필요한 행만 남긴다.

    기존 build_step 의미를 보존하려면 두 종류가 필요하다.
    1) 현재 ORDER_SEQ 이상이고 skip이 아닌 실제 후보 step
    2) DE_RANK 누적 계산에 필요한 S/Y step 전체(현재 이전 포함)

    따라서 단순 LOT_ID 필터보다 훨씬 강하게 줄이되 과거 S/Y는 보존한다.
    """
    before = len(df)
    lookup = {normalize_column_token(c): c for c in df.columns}
    lot_col = lookup.get(normalize_column_token("lot_id"))
    order_col = lookup.get(normalize_column_token("order_seq"))
    delay_col = lookup.get(normalize_column_token("delay_step_type"))
    skip_col = lookup.get(normalize_column_token("step_skip_yn"))
    missing = [name for name, col in (("lot_id", lot_col), ("order_seq", order_col),
                                      ("delay_step_type", delay_col), ("step_skip_yn", skip_col)) if col is None]
    if missing:
        raise RuntimeError(f"{line} StepPath 선필터 필수 컬럼 누락: {missing}")

    lot_key = df[lot_col].astype("string")
    current_order = lot_key.map(current_order_by_lot)
    order_num = pd.to_numeric(df[order_col], errors="coerce")
    delay = df[delay_col].astype("string")
    skip = df[skip_col].astype("string")

    target_lot = current_order.notna()
    rank_needed = delay.isin(["S", "Y"])
    candidate_needed = order_num.ge(current_order) & ~skip.eq("Y")
    keep = target_lot & (rank_needed | candidate_needed)

    filtered = df.loc[keep]
    after = len(filtered)
    stats.validations[f"{line}_step_local_filter_before"] = before
    stats.validations[f"{line}_step_local_filter_after"] = after
    stats.validations[f"{line}_step_local_filter_removed"] = before - after
    stats.validations[f"{line}_step_local_filter_ratio"] = round(after / max(before, 1), 4)
    print(f"[FILTER] {line} StepPath needed rows: {before:,} -> {after:,} ({before-after:,} removed)")
    del df
    gc.collect()
    return filtered


def build_tip_match_signatures(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    """현재 s에 실제로 존재하는 path로 매칭될 수 있는 TIP signature를 만든다.

    TrackInPrevent의 LOT_TYPE은 현재 전처리에서 항상 '-'이므로 LOT_TYPE bit(1)는 항상 wildcard다.
    PROCESS/STEP/PPID/EQPID 4개 컬럼의 16개 wildcard 조합별로 s의 가능한 equality key를 만든다.
    이후 대형 TrackInPrevent DataFrame을 이 signature와 한 번의 hash join으로 선필터한다.
    """
    con.execute("DROP TABLE IF EXISTS tip_match_signatures")
    con.execute(
        """
        CREATE TABLE tip_match_signatures (
            line VARCHAR,
            wildcard_mask INTEGER,
            k_process VARCHAR,
            k_step VARCHAR,
            k_ppid VARCHAR,
            k_eqpid VARCHAR
    ...
    for local_mask in range(16):
        full_mask = 1  # LOT_TYPE wildcard
        selects = ["LINE AS line"]
        predicates = []
        for local_bit, (tip_bit, scol, outcol) in enumerate(fields):
            if local_mask & (1 << local_bit):
                full_mask += tip_bit * 2  # 1,2,4,8 local -> 2,4,8,16 full
                selects.append(f"'' AS {outcol}")
            else:
                selects.append(f"CAST({scol} AS VARCHAR) AS {outcol}")
                predicates.append(f"{scol} IS NOT NULL")
        where = ("WHERE " + " AND ".join(predicates)) if predicates else ""
        con.execute(
            f"""
            INSERT INTO tip_match_signatures
            SELECT DISTINCT
                {selects[0]}, {full_mask} AS wildcard_mask,
                {selects[1]}, {selects[2]}, {selects[3]}, {selects[4]}
            FROM s
            {where}
            """
        )
    # 중복은 각 INSERT의 DISTINCT로 대부분 제거되지만 mask간에는 구분되므로 그대로다.
    sig_rows = query_table_count(con, "tip_match_signatures")
    stats.validations["tip_match_signature_rows"] = sig_rows
    print(f"[FILTER] TIP match signatures rows={sig_rows:,}")


def filter_and_stage_tip_dataframe(
    con: duckdb.DuckDBPyConnection,
    df: pd.DataFrame,
    table_name: str,
    line: str,
    ...
    """대형 TrackInPrevent를 현재 s에 매칭 가능성이 있는 규칙만 DuckDB에 적재한다.

    원천 SQL/TrackInPrevent wildcard 의미는 변경하지 않는다. 각 raw row의 wildcard mask와
    exact key를 만들고 s에서 생성한 동일 signature와 equality join한다.
    """
    expected = [
        "process", "step", "ppid", "eqpid", "chamberid", "type",
        "checkcount", "tkin_count", "updated", "eventtime",
    ]
    normalized = canonicalize_dataframe_columns(df, expected, table_name)
    before = len(normalized)

    # smoke는 샘플이 임의 LIMIT라 실제 s와 겹치지 않을 수 있으므로 기존처럼 그대로 적재한다.
    if smoke_test:
        stage_dataframe(con, normalized, table_name, stats, expected_columns=expected)
        stats.validations[f"{line}_tip_local_filter_before"] = before
        stats.validations[f"{line}_tip_local_filter_after"] = before
        stats.validations[f"{line}_tip_local_filter_removed"] = 0
        return

    con.register("_tip_stage_df", normalized)
    try:
        con.execute(f"DROP TABLE IF EXISTS {quote_ident(table_name)}")
        con.execute(
            f"""
            ...
            WITH raw AS (
                SELECT
                    *,
                    (1
                     + CASE WHEN COALESCE(NULLIF(TRIM(CAST(process AS VARCHAR)), ''), '-') = '-' THEN 2 ELSE 0 END
                     + CASE WHEN COALESCE(NULLIF(TRIM(CAST(step AS VARCHAR)), ''), '-') = '-' THEN 4 ELSE 0 END
                     + CASE WHEN COALESCE(NULLIF(TRIM(CAST(ppid AS VARCHAR)), ''), '-') = '-' THEN 8 ELSE 0 END
                     + CASE WHEN COALESCE(NULLIF(TRIM(CAST(eqpid AS VARCHAR)), ''), '-') = '-' THEN 16 ELSE 0 END
                    )::INTEGER AS wildcard_mask,
                    CASE WHEN COALESCE(NULLIF(TRIM(CAST(process AS VARCHAR)), ''), '-') = '-' THEN '' ELSE CAST(process AS VARCHAR) END AS k_process,
                    CASE WHEN COALESCE(NULLIF(TRIM(CAST(step AS VARCHAR)), ''), '-') = '-' THEN '' ELSE CAST(step AS VARCHAR) END AS k_step,
                    CASE WHEN COALESCE(NULLIF(TRIM(CAST(ppid AS VARCHAR)), ''), '-') = '-' THEN '' ELSE CAST(ppid AS VARCHAR) END AS k_ppid,
                    CASE WHEN COALESCE(NULLIF(TRIM(CAST(eqpid AS VARCHAR)), ''), '-') = '-' THEN '' ELSE CAST(eqpid AS VARCHAR) END AS k_eqpid
                FROM _tip_stage_df
            )
            SELECT
                r.process, r.step, r.ppid, r.eqpid, r.chamberid, r.type,
                r.checkcount, r.tkin_count, r.updated, r.eventtime
            FROM raw r
            INNER JOIN tip_match_signatures s
              ON s.line = '{line}'
             AND s.wildcard_mask = r.wildcard_mask
             AND s.k_process = r.k_process
             AND s.k_step = r.k_step
             AND s.k_ppid = r.k_ppid
             AND s.k_eqpid = r.k_eqpid
            """
        )
    finally:
        con.unregister("_tip_stage_df")

    after = query_table_count(con, table_name)
    stats.counts[f"{table_name}_rows"] = after
    stats.validations[f"{line}_tip_local_filter_before"] = before
    stats.validations[f"{line}_tip_local_filter_after"] = after
    stats.validations[f"{line}_tip_local_filter_removed"] = before - after
    stats.validations[f"{line}_tip_local_filter_ratio"] = round(after / max(before, 1), 4)
    print(f"[FILTER] {line} TIP matchable rules: {before:,} -> {after:,} ({before-after:,} removed)")


def drop_tables(con: duckdb.DuckDBPyConnection, names: Iterable[str]) -> None:
    """더 이상 사용하지 않는 작업 테이블을 즉시 제거한다."""
    for name in names:
...
# 원천 조회 및 staging
# =============================================================================
def load_base_sources(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    """작은 공통 원천(lot/equipment/eqp_group)만 먼저 조회한다."""
    ...
    stage_dataframe(
        ...
        expected_columns=["line_id", "origin_line_id", "batch_kind", "eqp_id", "eqp_status", "tool_kind", "eqp_status_change_time"],
    )
    del df; gc.collect()

    df = get_data(EQP_GROUP_QUERY, "eqp_group", stats)
    stage_dataframe(con, df, "eqp_group_raw", stats, expected_columns=["line_id", "eqp_group_name", "eqp_id"])
    del df; gc.collect()


def current_order_maps(con: duckdb.DuckDBPyConnection) -> dict[str, dict[str, int]]:
    out: dict[str, dict[str, int]] = {"KFR7": {}, "PFR1": {}}
    rows = con.execute(
        """
        SELECT LINE, LOT_ID, TRY_CAST(ORDER_SEQ AS BIGINT)
        FROM m
        WHERE TRY_CAST(ORDER_SEQ AS BIGINT) IS NOT NULL
        """
    ).fetchall()
    for line, lot_id, order_seq in rows:
        if line in out and lot_id is not None:
            out[line][str(lot_id)] = int(order_seq)
    return out


def load_step_sources(con: duckdb.DuckDBPyConnection, stats: RunStats, smoke_test: bool = False) -> None:
    order_maps = current_order_maps(con)
    # smoke에서 실제 lot 몇 개만 골라 DB 조회량 자체도 제한한다.
    smoke_ids = {line: list(mapping.keys())[:SMOKE_LOT_COUNT_PER_LINE] for line, mapping in order_maps.items()}
    for line, query, table in (
        ("KFR7", KFR7_STEP_PATH_QUERY, "step_kfr7_raw"),
        ("PFR1", PFR1_STEP_PATH_QUERY, "step_pfr1_raw"),
    ):
        actual_query = smoke_step_query(query, smoke_ids.get(line, []), SMOKE_STEP_ROWS_PER_LINE) if smoke_test else query
        df = get_data(actual_query, f"{line}_step_path", stats)
        df = filter_step_dataframe_to_needed_rows(df, order_maps.get(line, {}), line, stats)
        stage_dataframe(
            con, df, table, stats,
            expected_columns=[
...
def load_tip_sources(con: duckdb.DuckDBPyConnection, stats: RunStats, smoke_test: bool = False) -> None:
    build_tip_match_signatures(con, stats)
    for line, query, table in (
        ...
        # raw 전체를 DuckDB에 먼저 복사하지 않고 pandas view에서 바로 matchable rule만 물리화한다.
        filter_and_stage_tip_dataframe(con, df, table, line, stats, smoke_test)
        del df; gc.collect()
...
def build_dimensions(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    ...
    eqpg = con.execute(
        """
        SELECT COUNT(*), COALESCE(MAX(eqp_count), 0)
        FROM (
    ...
    stats.validations["eqp_group_multi_eqp_group_count"] = int(eqpg[0])
    stats.validations["eqp_group_max_eqp_count"] = int(eqpg[1])
...
def build_tip(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    ...
    multi = con.execute(
        """
        SELECT COUNT(*), COALESCE(MAX(chamber_rows), 0)
        FROM (
    ...
    stats.validations["tip_multi_chamber_key_count"] = int(multi[0])
    stats.validations["tip_max_chamber_rows_per_key"] = int(multi[1])
...
def build_t_matches_partitioned(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    """TIP wildcard 규칙을 OR 조인 대신 wildcard mask별 equality(hash) join으로 분해한다.

    '-' / NULL / blank인 컬럼만 wildcard이며, 나머지 컬럼은 모두 AND 일치해야 한다.
    LOT_TYPE, PROCESS, STEP, PPID, EQPID 5개 컬럼을 bit mask(0~31)로 분류한다.
    """
    con.execute("DROP TABLE IF EXISTS t_match_rules")
    con.execute(
        """
        CREATE TABLE t_match_rules AS
        SELECT
            t.*,
            (
                CASE WHEN COALESCE(NULLIF(TRIM(CAST(LOT_TYPE AS VARCHAR)), ''), '-') = '-' THEN 1 ELSE 0 END +
                CASE WHEN COALESCE(NULLIF(TRIM(CAST(PROCESS AS VARCHAR)), ''), '-') = '-' THEN 2 ELSE 0 END +
                CASE WHEN COALESCE(NULLIF(TRIM(CAST(STEP AS VARCHAR)), ''), '-') = '-' THEN 4 ELSE 0 END +
                CASE WHEN COALESCE(NULLIF(TRIM(CAST(PPID AS VARCHAR)), ''), '-') = '-' THEN 8 ELSE 0 END +
                CASE WHEN COALESCE(NULLIF(TRIM(CAST(EQPID AS VARCHAR)), ''), '-') = '-' THEN 16 ELSE 0 END
            ...
            CASE
                WHEN COALESCE(NULLIF(TRIM(CAST(PROCESS AS VARCHAR)), ''), '-') <> '-'
                 AND COALESCE(NULLIF(TRIM(CAST(STEP AS VARCHAR)), ''), '-') <> '-'
                 AND COALESCE(NULLIF(TRIM(CAST(PPID AS VARCHAR)), ''), '-') <> '-'
                 AND COALESCE(NULLIF(TRIM(CAST(EQPID AS VARCHAR)), ''), '-') <> '-'
                THEN 0 ELSE 1
            END AS priority
        FROM t
        """
    ...
    mask_rows = con.execute(
        """
        SELECT wildcard_mask, COUNT(*)
        FROM t_match_rules
        GROUP BY wildcard_mask
        ORDER BY wildcard_mask
        """
    ).fetchall()
    stats.validations["tip_wildcard_mask_rule_counts"] = {str(mask): int(cnt) for mask, cnt in mask_rows}
    stats.validations["tip_wildcard_rule_rows"] = int(sum(cnt for mask, cnt in mask_rows if mask != 0))
    stats.validations["tip_exact_rule_rows"] = int(sum(cnt for mask, cnt in mask_rows if mask == 0))

    con.execute("DROP TABLE IF EXISTS t_matches_candidates")
    con.execute(
        """
        CREATE TABLE t_matches_candidates AS
        SELECT
            CAST(NULL AS BIGINT) AS ms_row_id,
            t.*,
    ...
    for mask, rule_count in mask_rows:
        predicates = ["ms.LINE = r.LINE"]
        for bit, rule_col, ms_col in bit_defs:
            if not (mask & bit):
                predicates.append(f"ms.{ms_col} = r.{rule_col}")
        join_sql = "\n             AND ".join(predicates)
        print(f"[F1][TIP MATCH] wildcard_mask={mask:02d}, rules={rule_count:,}")
        con.execute(
            f"""
            INSERT INTO t_matches_candidates
            SELECT
                ms.ms_row_id,
                r.* EXCLUDE (wildcard_mask)
            FROM ms_joined ms
            INNER JOIN t_match_rules r
              ON {join_sql}
            WHERE r.wildcard_mask = {int(mask)}
            """
        )

    stats.validations["tip_match_candidate_rows"] = query_table_count(con, "t_matches_candidates")
    stats.validations["tip_match_candidate_amplification_vs_ms"] = round(
        stats.validations["tip_match_candidate_rows"] / max(query_table_count(con, "ms_joined"), 1), 4
    )
    ...
    con.execute(
        ...
        FROM (
            ...
            FROM t_matches_candidates c
        ) x
        WHERE rn = 1
        """
    )
    stats.validations["tip_match_rows_after_semantic_dedup"] = query_table_count(con, "t_matches")
    stats.validations["tip_match_semantic_duplicates_removed"] = (
        stats.validations["tip_match_candidate_rows"]
        - stats.validations["tip_match_rows_after_semantic_dedup"]
    )


def build_f1(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    with timer(stats, "f1-1 m_base"):
        con.execute("DROP TABLE IF EXISTS m_base")
        con.execute("CREATE TABLE m_base AS SELECT ROW_NUMBER() OVER () AS m_row_id, * FROM m")

    with timer(stats, "f1-2 m x s join"):
    ...
    with timer(stats, "f1-3 TIP wildcard partitioned join"):
        build_t_matches_partitioned(con, stats)
...
def collect_row_change_validations(con: duckdb.DuckDBPyConnection, stats: RunStats) -> None:
    ...
    # 민감정보 보호: 실제 line/process/step/eqp id 샘플은 로그에 남기지 않는다.
    # 증폭 진단은 build_dimensions/build_tip에서 집계 count/max만 기록한다.
...
def write_log(stats: RunStats) -> Path:
    ...
    lines = [
        ...
        "- StepPath는 원천 SQL 전체 조회 유지 후 pandas 반환 직후 현재 재공 LOT만 로컬 선필터",
        "- 단계 종료 시 불필요한 중간 DuckDB 테이블 즉시 DROP",
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        with timer(stats, "기본 원천 조회 및 staging"):
            load_base_sources(con, stats)
        with timer(stats, "공통 dimension 생성"):
            build_dimensions(con, stats)
            drop_tables(con, ["lot_raw", "equipment_raw", "eqp_group_raw"])
        with timer(stats, "StepPath 조회 및 선필터"):
            load_step_sources(con, stats, smoke_test=args.smoke_test)
        with timer(stats, "Step 전처리"):
            build_step(con, stats)
            print(f"[ROWS] s rows={stats.counts.get('s_rows', 0):,}")
            drop_tables(con, ["step_kfr7_raw", "step_pfr1_raw", "step_path", "current_lot", "step_rank", "step_candidate", "step_with_group"])
        with timer(stats, "TrackInPrevent 조회 및 matchable 선필터"):
            load_tip_sources(con, stats, smoke_test=args.smoke_test)
        with timer(stats, "Tip 전처리"):
            build_tip(con, stats)
            print(f"[ROWS] t rows={stats.counts.get('t_rows', 0):,}")
            drop_tables(con, ["tip_kfr7_raw", "tip_pfr1_raw", "tip_raw_union", "tip_t", "tip_ttt", "tip_te", "tip_main", "tip_cham", "tip_tee", "equipment_status", "tip_match_signatures"])
        with timer(stats, "f1 생성"):
            build_f1(con, stats)
            drop_tables(con, ["m_base", "ms_joined", "t_match_rules", "t_matches_candidates", "t_matches", "f1_base", "f1_counts", "f1_groups", "f1_status_base", "f1_current", "f1_blocked_rank", "f1_raw"])
            # f1 이후에는 s/t가 더 이상 필요 없다. m은 최종 검증의 LOT 기준으로 유지한다.
            drop_tables(con, ["s", "t"])
        with timer(stats, "f2 생성"):
            build_f2(con, stats)
            drop_tables(con, ["f2_tip_parts", "f2_tip_summary", "f2_down_parts", "f2_down_grouped", "f2_down_summary", "f2_eqpline_values", "f2_eqpline_summary", "f2_calc", "f2_final"])
