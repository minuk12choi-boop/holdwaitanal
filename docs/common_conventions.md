...
재공 구성 카드 우측 상단의 `제품별 비율` 버튼. 전체 막대 아래에 회색 점선을
긋고, 한 단 들여 쓴 자리에 제품명을 두고 제품마다 같은 모양의 막대를 놓는다.
**재공이 많은 제품이 위**로 온다. 색은 전체 막대와 같은 lot_type 색을 쓴다.

조각을 누르면 **(제품, lot_type) 을 한 쌍**으로 건다(`iprod` + `type`).
Ctrl 로 여러 조각을 고를 수 있고, 다른 차트에서 조건을 걸면 이 목록도
그만큼 좁혀진다. 다만 **제 축(제품·lot_type)은 늘 전부** 남는다.
...
`base.html` 이 `window.fetch` 를 감싸 **진행 중인 요청을 센다.** 하나라도
있으면 화면 전체를 반투명하게 덮고 원을 돌린다. 페이지마다 따로 부를
필요가 없고, 반쯤 그려진 화면이 보이지 않는다.
...
```
shape()      구간별 감소율 · 시작일 · 지속 · 재공 변화
cause_at()   그 시점 f3_history 에서 원인을 읽는다
group_findings()  설비성은 AREA 로, LOT 성은 제품>PLAN>모듈>LAYER 로 묶는다
phrase()     문장 생성
summarize()  상위 3 + 나머지 규모
```

**문구 규칙**

```
어휘   감소 · 증가만. 추상 표현을 쓰지 않는다
수치   비율과 매수를 함께. 비교 기준(N일 평균)을 반드시 밝힌다
주어   모든 문장에 주어가 있다. '5일째입니다' 같은 서술은 만들지 않는다
고정   같은 스냅샷이면 같은 문장(스냅샷 시각이 씨앗)
```
...
TIP 조인은 **left** 로 한다. 설비를 못 찾은 스텝도 남겨야 한다. inner 로
하면 그 스텝이 사라지고, 현스텝이 거기 걸린 lot 은 화면에서 통째로 빠진다.

자기 라인과 호환 라인 설비를 **함께** 담되 순서는 자기 라인이 앞이다.
`line_rank`(0=자기 · 1=호환)를 f1_base 까지 넘겨 STRING_AGG 정렬에 쓴다.

### UPDATE 는 PRIMARY KEY 로

컬럼에 함수를 씌우면 인덱스를 못 쓴다. 행마다 전체 스캔이 된다.

```
느림   WHERE snapshot_at=? AND lot_id=? AND COALESCE(step_seq,'')=COALESCE(?,'')
빠름   WHERE id IN (...)
```

2만 행 · 컬럼 하나에 **13.6초 대 0.07초** 였다(sqlite 기준, MySQL 원격은
더 벌어진다).

같은 값으로 바뀌는 행이 많으므로 **값별로 묶어 IN** 으로 보낸다.
UPDATE 횟수가 행 수가 아니라 서로 다른 값의 수로 줄어든다.

### 종료 코드

배치가 실패해도 종료 코드가 0 이면 스케줄러는 성공으로 본다. 며칠씩
옛 데이터를 보고 있어도 아무도 모른다.

```
build_f3.py    DB 적재 실패 -> FAILED 에 담고 sys.exit(1)
run_pipeline   한 단계라도 실패하면 exit /b 1
```

**배치 파일 함정** — `if (...)` 블록 안의 `%ERRORLEVEL%` 는 블록에
들어갈 때 한 번 펼쳐진다. 명령을 실행한 뒤 값이 아니라 **이전 값**이
찍힌다. `call :label` 로 빼서 명령 직후에 읽어야 한다.

### 놓친 SHIFT 메우기

`promote_to_history` 는 직전 SHIFT 하나만 보면 안 된다. 파이프라인이
몇 시간 멈춘 사이 지나간 SHIFT 는 영영 이력이 빈다.

```
아직 f3_history_meta 에 없는 것 중 **가장 오래된 것**부터 채운다.
...
`f3_history` 는 하루 6만 행 남짓 쌓인다. 1년이면 2천만 행이 넘고
디스크가 20GB 가까이 된다. 그대로 두면 조회가 느려진다.

```
HISTORY_KEEP_DAYS = 400      db_common.py
```

적재할 때마다 그보다 오래된 것을 지운다. `f3_wip_step` ·
`f3_move_step` · `f3_move_lot` 도 함께 정리한다.
화면 목록은 60일치만 쓰므로 넉넉하다.

### 값이 잘리는 컬럼

`TEXT_COLUMNS` 가 아닌 컬럼은 **128자에서 잘린다.** 조용히 넘기면
화면에서 값이 잘린 줄 모르므로 적재할 때 남긴다.

```
[CUT] 128자를 넘어 잘린 값: recipe_id 3건  (자주 잘리면 TEXT_COLUMNS 에)
```
...
```
NRD   KFR4  #3D3C3A      NRD-K  KFR7  #045F5F
NRD-P PFR1  #3D5791      NRD-V  KFR6  #667C26
```

누적막대의 값 라벨은 **계열 합계**를 쓴다. 첫 계열만 읽으면 라인 ALL 에서
한 라인 값만 보인다. y축 최대도 합계로 다시 잡아야 막대가 경과일 라인
대역(68~90%)을 침범하지 않는다.

### 설비는 전 라인으로도 본다

Bottleneck 과 설비이슈는 **같은 설비를 여러 라인이 나눠 쓴다.** 제 라인만
봐서는 그 설비가 왜 막혔는지 알 수 없다.

라인 필터에 `ALL` 을 두는 방식은 되돌렸다. 모든 차트가 라인 누적으로
바뀌면서 조작이 얽혀 문제가 많았다. 대신 **요약카드의 설비 카드
(B/N · 설비 이슈) 우측 상단에 `전 라인` 토글**을 둔다.

```
켜면  그 설비 앞에 선 재공을 라인 구분 없이 세어 한 줄 덧붙인다
      전 라인 7.2k매 · NRD 2.4k · NRD-P 2.4k · NRD-K 2.4k
```

`/api/eqp-wait/` 가 라인마다 조회해 설비 단위로 합친다. 화면의 다른
부분은 그대로 제 라인만 본다.

### 차트는 혼자 걸렸을 때만 전체를 보인다

그 차트에서만 골랐다면 그 차트는 전체를 유지해 다시 고를 수 있게 한다.
**다른 조건이 하나라도 더 걸리면 모든 차트가 함께 좁혀진다**
(원차트에서 WAIT 를 고르고 AREA 도 고르면 원차트도 WAIT 만 남는다).

### 구역 구분

좌측(드릴다운을 거는 곳)과 우측(그 결과)을 굵은 선으로 가른다
(`.maincol { border-left: 3px }`). 요약카드와 그 아래도 마찬가지다.

### 업데이트 내역

`/master/` 위쪽의 `업데이트 내역 보기` 버튼 -> 팝업. 실제로 반영된 것만
적는다. 목록은 `views.py` 의 `UPDATES` 다.

기준정보(`/master/`)는 **메뉴에 두지 않는다.** 주소로만 들어간다.
SSO 로그인을 붙이면 다시 올리고, 허용된 사용자만 들어가게 한다
(권한 카드를 기준정보에 새로 만들 예정).

```
released   그냥 항목. 지금 쓸 수 있다
dropped    되돌린 항목. 회색 음영 + 취소선 + '취소' 배지
superseded 무엇으로 대체됐는지. 취소 항목 뒤에 화살표로 잇는다
note       DB 작업이 더 필요한 경우 그 파일 이름
```

새 기능을 넣으면 **그 자리에서 UPDATES 에도 한 줄 추가**한다.
되돌릴 때는 지우지 말고 `dropped` 로 바꾸고 `superseded` 를 적는다.
...
### 차트는 자기 필드를 뺀 나머지 조건으로 그린다

원차트에서 HOLD 를 고르면 재공막대 · W/T · W/T 0 · 원인 분석 · 요약카드는
모두 HOLD 로 좁혀지고, **원차트 자신만 다른 status 를 계속 보여 준다**
(안 그러면 다시 고를 수 없다). LOT BALANCE 는 고른 LAYER 만 남기고
나머지 막대를 감춘다(축 자리와 굵기는 유지).

요약카드는 **카드에서 고른 조건(`x_*`) 만** 무시한다. 그 외 조건은 따른다.
