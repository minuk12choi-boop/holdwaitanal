...
# 업데이트 내역
# ---------------------------------------------------------------------------
#   released : 실제로 반영돼 지금 쓸 수 있는 항목
#   dropped  : 만들었다가 되돌린 항목. 화면에 취소선으로 남긴다.
#              superseded 에 무엇으로 대체됐는지 적는다.
#   note     : 적용에 필요한 추가 작업(마이그레이션 등)
UPDATES = [
    {
        "ver": "v0.9",
        "date": "",
        "title": "화면 뼈대와 기준정보",
        "items": [
            {"t": "FAB현황 · 기준정보 · 다운로드 세 화면을 갖췄습니다."},
            {"t": "기준정보를 모듈 · HOLD 유형 · 초HOT · 제품별 메인 PLAN "
                  "네 가지로 나눠 관리합니다."},
            {"t": "기준정보를 고치면 `지금 반영` 으로 즉시 적용합니다."},
        ],
    },
    {
        "ver": "v1.0",
        ...
        "title": "드릴다운 기본 구조",
        "items": [
            {"t": "좌측 차트(재공 · STATUS · W/T · W/T 0)에서 고른 조건이 "
                  "우측 전체에 이어집니다."},
            {"t": "원인 분석을 Hold · Wait성 진행불가 · Bottleneck 세 갈래로 "
                  "나눠 보여 줍니다."},
            {"t": "LOT BALANCE 를 LAYER 축의 STATUS 누적막대로 그립니다."},
            {"t": "드릴다운 표에 34개 컬럼과 정렬 · 필터 · 복사를 넣었습니다."},
            {"t": "SHIFT 추이 차트로 시간대별 흐름을 봅니다.",
             "dropped": True,
             "superseded": "v1.4 라인 요약 카드"},
        ],
    },
    {
        "ver": "v1.1",
        ...
        "title": "원인 분류 정리",
        "items": [
            {"t": "원인을 상태의 하위 분류로 바로잡았습니다. "
                  "HOLD · WAIT(진행불가) · WAIT 순으로 먼저 나눈 뒤 "
                  "그 안에서 세부 원인을 가립니다."},
            {"t": "Bottleneck 계층을 AREA > 설비 ID 로 바꿨습니다."},
            {"t": "설비 이슈는 PM · DOWN · LOCAL 코드를 접두로 씁니다."},
            {"t": "가상스텝(설비그룹 · RECIPE · STEP 에 WAIT 포함, "
                  "NRDSEND · NRDMEAS)을 WAIT(진행불가)로 봅니다.",
             "note": "기존 적재분은 migrate_virtual_step_status.sql 과 "
                     "migrate_virtual_eqp.sql 을 실행합니다."},
            {"t": "호환설비이슈를 Bottleneck 으로 분류해 드릴다운이 "
                  "이어지도록 고쳤습니다."},
            {"t": "lot 단위로 접을 때 현스텝 행의 값을 쓰도록 바로잡았습니다. "
                  "연속블록 값이 섞여 상태와 LAYER 가 어긋나던 문제입니다."},
        ],
    },
    {
        "ver": "v1.2",
        ...
        "title": "조건 규칙 통일",
        "items": [
            {"t": "조건을 차트가 아니라 필드 단위로 다룹니다. "
                  "여러 차트가 같은 STATUS 를 공유합니다."},
            {"t": "같은 필드의 여러 값은 또는, 다른 필드끼리는 그리고 로 겁니다."},
            {"t": "같은 필드를 다른 차트에서 고르면 교집합으로 좁힙니다."},
            {"t": "해제는 같은 차트에서 같은 값을 다시 누를 때만 일어납니다."},
            {"t": "조합 결과가 없으면 0 을 그대로 보여 줍니다. "
                  "한쪽을 임의로 풀지 않습니다."},
            {"t": "각 차트는 자기 축을 뺀 조건으로 그립니다. "
                  "고른 것은 테두리, 나머지는 흐리게 표시해 "
                  "언제든 다시 고를 수 있습니다."},
            {"t": "누적막대는 구간과 조각을 한 쌍으로 다룹니다."},
        ],
    },
    {
        "ver": "v1.3",
        ...
        "title": "원인 분석 드릴다운",
        "items": [
            {"t": "하위가 있는 항목을 누르면 전체 보기와 세부 요소 보기를 "
                  "고르는 말풍선이 뜹니다."},
            {"t": "세부 요소로 파고들면 원래 차트는 그대로 두고 "
                  "아래에 차트가 한 장씩 쌓입니다."},
            {"t": "설비 그룹 아래 설비 ID 까지 내려갑니다."},
            {"t": "더 쪼갤 수 없는 항목은 차트를 만들지 않고 "
                  "표와 LOT BALANCE 만 바뀝니다."},
            {"t": "전체 순위를 넓은 패널로 따로 봅니다."},
            {"t": "세 대분류를 합친 전체 제약 차트",
             "dropped": True,
             "superseded": "v1.4 라인 요약 카드"},
        ],
    },
    {
        ...
        "items": [
            ...
            {"t": "비율을 라인 전체 대비와 그 제품 안에서의 비중 "
                  "두 가지로 적습니다."},
            {"t": "STATUS 원차트에 매수 / LOT 토글을 더했습니다. 기본은 "
                  "매수이며, 조각 크기와 비율이 고른 기준을 따릅니다."},

            {"t": "Bottleneck 은 순수 몰림인지 호환 그룹이 막혀서인지 "
                  "배지로 구분합니다."},
            {"t": "행을 누르면 그 조건으로 화면 전체가 좁혀집니다. "
                  "이때 다른 곳에서 건 조건은 그대로 남습니다."},
            {"t": "전체 순위 버튼으로 한 카드만 남기고 20위까지 봅니다."},
            {"t": "설비 카드에는 전 라인 토글이 있어 다른 라인 재공까지 "
                  "함께 셉니다."},
            {"t": "전일 · 직전 SHIFT 비교 토글",
             "dropped": True,
             "superseded": "비교 없이 현재 값만 보도록 정리"},
        ],
    },
    {
        "ver": "v1.5",
        ...
        "title": "차트 보강",
        "items": [
            {"t": "AREA 별 재공 카드를 더했습니다."},
            {"t": "재공 구성에 제품별 비율 보기를 더했습니다. "
                  "조각을 눌러 제품과 TYPE 을 함께 겁니다."},
            {"t": "LOT BALANCE 를 제품 · PLAN 으로 나눠 봅니다. "
                  "제품으로 나누면 그 제품의 LAYER 만 축에 둡니다."},
            {"t": "LOT BALANCE x축을 LAYER · 모듈2 · 모듈1 세 단으로 나눴습니다."},
            {"t": "AREA 필터를 최상단에 더했습니다."},
            {"t": "라인 필터의 ALL 선택",
             "dropped": True,
             "superseded": "v1.4 설비 카드의 전 라인 토글"},
            {"t": "카드 사이를 잇는 흐름 화살표",
             "dropped": True,
             "superseded": "여백이 좁아 효과가 없어 카드 테두리 표시만 남김"},
        ],
    },
    {
        "ver": "v1.6",
        ...
        "title": "기준정보 · 진단",
        "items": [
            {"t": "HOLD 유형에 적용 순서를 넣었습니다. 위에 있는 행이 먼저 "
                  "걸리고, 손잡이를 끌거나 번호를 직접 넣어 바꿉니다.",
             "note": "migrate_holdtype_sort.sql 을 실행합니다."},
            {"t": "CONDITION 은 대문자로 저장하고 대소문자를 가리지 않고 "
                  "맞춥니다."},
            {"t": "HOLD 사유에서 반복되는 문구를 찾아 기준정보 조건 후보를 "
    ...
    },
    {
        ...
        "items": [
            ...
            {"t": "필터끼리 서로 선택지를 좁혀 줍니다. 고를 수 없는 값은 "
                  "목록에 나오지 않습니다."},
            {"t": "달력에서 데이터가 없는 날짜는 회색으로 표시되고 선택되지 "
    ...
    },
    {
        "ver": "v1.8",
        "date": "",
        "title": "NRD-P FabPlan 재공",
        "items": [
            {"t": "NRD-P 재공 중 공정 순서가 없는 lot(FabPlan)도 스텝 경로를 "
                  "붙여 화면에 나오게 했습니다."},
            {"t": "EIN · ECN · RCS 규제와 사전지정을 반영해 진행 가능한 "
                  "설비와 RECIPE 를 가립니다."},
            {"t": "연속 구간은 딜레이 제약으로 판정합니다."},
        ],
    },
    {
        "ver": "v1.7",
        ...
        "title": "속도와 화면 정리",
        "items": [
            {"t": "조회 결과를 스냅샷 단위로 캐시해 응답을 크게 줄였습니다."},
            {"t": "요약을 표보다 먼저 불러오고, 지난 요청은 취소합니다."},
            {"t": "드릴다운 표를 200행씩 나눠 그려 멈춤을 없앴습니다. "
                  "정렬 · 필터 · 복사는 전체 행을 그대로 씁니다."},
            {"t": "첫 화면은 다 그려질 때까지 덮어 두고, 이후 조작은 "
                  "상단 진행 바와 카드 흐림으로만 알립니다."},
            {"t": "좌측과 우측, 요약카드와 그 아래를 굵은 선으로 나눴습니다."},
            {"t": "카드마다 해제 버튼을 두고, 누를 수 있는 곳은 "
                  "손가락 커서로 바꿨습니다."},
        ],
    },
]
...
def _sub_name(r, ht, kind, reason_col, rules, cat):
    ...
    if pre:
        return pre
    if ht:
        # 종류(kind)와 사유 컬럼은 이미 정해져 넘어온다. 그것만 본다.
        #   여기서 다시 세 컬럼을 훑으면 HOLD 인 lot 이 예약제외 규칙에
        #   걸리는 등 종류가 규칙 순서에 휘둘린다.
        got = match_holdtype(r, ht, kind, reason_col)
        if got:
            return got
